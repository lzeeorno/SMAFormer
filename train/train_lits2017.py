...
def parse_args():
    ...
    parser.add_argument('--model_name', default='SMAFormer',
                        choices=['Unet', 'AttUnet', 'res_unet_plus', 'R2Unet', 'R2AttU_Net',
                                 'sepnet', 'KiU_Net', 'Unet3D', 'ResT', 'SMAFormer','AFFNet'
                                 ])
    # pre trained
    ...
    parser.add_argument('--lr', '--learning-rate', default=0.01, type=float,
                        metavar='LR', help='initial learning rate, Resunet=1e-4, R2Unet=1e-5, Unet=1e-2, ResUformer=0.001,AFF=0.01')
    parser.add_argument('--interval', '--train_interval', default=224, type=float,
                        metavar='LR', help='interval to change the lr')
...
def train(args, train_loader, model, criterion, optimizer, lr_decay, epoch, index):
    ...
    l2_reg = 0.5
    loss_list = []
    for iter, (input, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if iter % args.interval == 0 and iter != 0:
...
def main():
    ...
    if args.model_name == 'SMAFormer':
        ...
        pretrained_path = '../trained_models/LiTS2017_SMAFormer/2025-02-14-18-33-58/epoch105-val_loss:0.1621-val_dice2:0.6399_model.pth'
