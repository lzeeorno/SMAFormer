...
class SMAFormerV2(nn.Module):
    def __init__(self, args):
        ...
        self.modulator1 = Modulator(filters[1], filters[1])
        ...
        self.modulator2 = Modulator(filters[3], filters[3])
        ...
        self.modulator3 = Modulator(filters[5], filters[5])
        ...
        self.modulator_fusion1 = Modulator(filters[5], filters[5])
        ...
        self.up_residual_conv1 = ResidualConv(filters[5] + filters[4], filters[5], 1, 1)
        ...
        self.modulator_fusion2 = Modulator(filters[1], filters[1])
    ...
    def forward(self, x):
        ...
        # x2_skip = self.modulator1.CA(x2) #same
        ...
        # x4_skip = self.modulator3.CA(e3)
        # x4 = self.residual_conv3(x4_skip) #[16,512,16,16]->[16,512,16,16]

        # b, c, h, w = x4.size()  #[16,512,16,16]->[16,16*16,512]
        # x5 = x4.view(b, c, h * w).contiguous().permute(0, 2, 1)  # 调整形状以适应Transformer模块
        e4 = self.EncoderBlock4(e3, e3)  # [16, 16*16(h*w), c=512]
        # b, hw, c = e4.size()
        # h = w = int(hw ** 0.5)
        # x5 = e4.permute(0, 2, 1).view(b, c, h, w)  #调整回卷积形状 #[16, 16*16(h*w), c=512]->[16,512,16,16]
        # x_bridge = self.modulator_fusion1.forward(x5)  #same
        # b, c, h, w = x_bridge.size()
        # x_bridge = x_bridge.view(b, c, h * w).contiguous().permute(0, 2, 1)
        ...
        x6 = self.upsample1(x5) #[16,512,16,16]->[16,512,32,32]
        x6 = torch.cat([x6, x3], dim=1) #[16,512+256,32,32]
        x6 = self.up_residual_conv1(x6) #[16,512+256,32,32]->[16,512,32,32]
        b, c, h, w = x6.size()
        x6 = x6.view(b, c, h * w).contiguous().permute(0, 2, 1) ##[16,32*32,512]
        b, num_patch, c = e3.size()
        e3 = e3.view(b, c, int(num_patch ** 0.5), int(num_patch ** 0.5))  # [16,16*16,512]->[16,512,16,16]
        e3 = self.upsample1(e3) #[16,512,16,16]->[16,512,32,32]
        b, c, h, w = e3.size()
        ...
        x7 = self.upsample2(x6) #[16, 512, 32, 32]->[16, 512, 128, 128]
        ...
        b, c, h, w = x7.size()
        x7 = x7.view(b, c, h * w).contiguous().permute(0, 2, 1)  ##[16,128,128,128]->[16,128*128,128]
        e2 = self.upsample1(e2)  #[16,128,64,64]->[16,128,128,128]
        b, c, h, w = e2.size()
        e2 = e2.view(b, c, h * w).contiguous().permute(0, 2, 1)  ##[16,128*128,128]
        x7 = self.DecoderBlock3(x7, e2) #[16,128*128,128]skip_conn[16,128*128,128]
        b, hw, c = x7.size()
        h = w = int(hw ** 0.5)
        x7 = x7.permute(0, 2, 1).contiguous().view(b, c, h, w)  #[16,128*128,128]->[16,128*128,128]

        x8 = self.upsample3(x7) #[16,128,128,128]->[16,128,512,512]
        ...
        # x9 = self.modulator_fusion2.forward(x8) #[16,32,512,512]->[16,16,512,512]
...
exp6: ViT -> SMAFormer
success, but attn fusion cannot be added


exp7: delete all SE, ASPP, replace into Embedded_modulator.Synergistic_attn(MSA+(PA@CA@SA))
