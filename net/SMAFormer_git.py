...
class E_MLP(nn.Module):
    def __init__(self, embed_dim, forward_expansion, dropout):
        ...
        self.depthwise_conv = nn.Conv2d(
            ...
            out_channels=forward_expansion * embed_dim,
            kernel_size=3,
            padding=1,
            groups=forward_expansion * embed_dim  # Grouped convolution
        )

        # Replaced 3x3 convolution with 1x1 pointwise convolution to reduce FLOPs and params
        ...
        self.linear2 = nn.Linear(forward_expansion * embed_dim, embed_dim)

    def forward(self, x):
        b, hw, c = x.size()
        feature_size = int(math.sqrt(hw))

        x = self.linear1(x)  # x: (b, hw, c) -> (b, hw, forward_expansion * c)
        x = self.act(x)
